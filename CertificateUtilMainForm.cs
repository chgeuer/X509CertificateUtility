namespace X509CertificateTool
{
    ...
    public partial class CertificateUtilMainForm : Form
    {
        ...
        public CertificateUtilMainForm()
        {
            ...
            this.textBoxRegex.Text = this.Regex;
        }
        ...
        internal static readonly List<StoreName> _allStoreNames = new List<StoreName>();
        internal static readonly List<StoreLocation> _allStoreLocations = new List<StoreLocation>();
        ...
        private bool regexMatchCase
        {
            get { return X509CertificateTool.Properties.Settings.Default.regexMatchCase; }
            set
            {

                X509CertificateTool.Properties.Settings.Default.regexMatchCase = value;
                X509CertificateTool.Properties.Settings.Default.Save();
            }
        }

        private string Regex
        {
            get { return X509CertificateTool.Properties.Settings.Default.regex; }
            set
            {
                X509CertificateTool.Properties.Settings.Default.regex = value;
                X509CertificateTool.Properties.Settings.Default.Save();
            }
        }
        ...
        {
            Collection<CertData> result = new Collection<CertData>();
        ...
        {
            StoreSelectionForm ssf = new StoreSelectionForm(
                CertificateUtilMainForm._allStoreLocations,
                CertificateUtilMainForm._allStoreNames,
                this._selectedStoreLocations,
                this._selectedStoreNames,
                this.regexMatchCase);

            ssf.ShowDialog();

            this._selectedStoreLocations.Clear();
            this._selectedStoreLocations.AddRange(ssf.StoreLocations);

            this._selectedStoreNames.Clear();
            this._selectedStoreNames.AddRange(ssf.StoreNames);

            this.regexMatchCase = ssf.RegexMatchCase;
        }
        ...
        private void buttonRegexGo_Click(object sender, EventArgs e)
        {
            ...
            this.Regex = regexText;

            Regex regex;
            RegexOptions regexOptions = this.regexMatchCase ?
            ...
            string operation)
        {
            ...
            filename = getFilenameSuggestionForDeletedCertificate(
                cert, temporaryCertificateExportFolder, extension, operation);

            byte[] certOctets = null;
            if (certData.CertHasPrivateKey && certData.PrivateKeyIsExportable)
            {
